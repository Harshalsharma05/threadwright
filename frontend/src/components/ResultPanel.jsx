...
const TermRow = ({ label, value, delay }) => (
    <div className="flex flex-col sm:flex-row sm:gap-4 term-animate" style={{ animationDelay: delay }}>
...
export default function ResultPanel({ runId, synthesizeStatus, company }) {
    ...
    useEffect(() => {
        ...
        } else if (synthesizeStatus === 'running' || synthesizeStatus === 'pending') {
        ...
        <div className="mt-8" ref={terminalRef}>
            ...
            <div className="bg-[#0f172a] rounded-lg border border-slate-700 shadow-2xl overflow-hidden font-mono text-sm min-h-[400px]">
                ...
                <div className="bg-[#1e293b] px-4 py-2 border-b border-slate-700 flex items-center justify-between sticky top-0 z-10">
                    ...
                    <span className="text-slate-500 text-xs">threadwright@engine: ~/output</span>
                </div>
                    ...
                    {error ? (
                        ...
                        <>
                            <div className="text-emerald-500 font-bold mb-6">
                                $ ./synthesize_intel.sh --target="AWAITING_INPUT"<span className="cursor-blink"></span>
                            </div>
                            <TerminalSkeleton />
                        </>
                    ) : (
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="text-emerald-500 font-bold term-animate" style={{ animationDelay: '0ms' }}>
                                $ ./synthesize_intel.sh --target="{company || 'UNKNOWN'}"
                                <br/>
                                <span className="text-slate-400 font-normal">STATUS: 200 OK | PAYLOAD: DECRYPTED</span>
                            </div>

                            <div className="border-l-2 border-indigo-500 pl-4 py-2 bg-indigo-950/30 term-animate" style={{ animationDelay: '100ms' }}>
                                <div className="text-indigo-400 mb-2"># STRATEGY_OVERRIDE</div>
                                <p className="text-slate-300 leading-relaxed">
                                    <TypewriterText text={result.strategic_interview_advice} delayOffset={200} highlightKeywords={keywords} />
                                </p>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-y border-slate-800 py-4 term-animate" style={{ animationDelay: '1500ms' }}>
                                <div className="space-y-2">
                                    <div className="text-slate-500 mb-2"># MARKET_METRICS</div>
                                    <TermArray label="COMPETITORS" items={result.market_and_competition?.direct_competitors} colorClass="border-slate-600 text-slate-400" delay="1600ms" />
                                    <TermRow label="POSITION" value={<TypewriterText text={result.market_and_competition?.market_position_and_challenges} delayOffset={1700} highlightKeywords={keywords} />} delay="1700ms" />
                                </div>
                                <div className="space-y-2">
                                    <div className="text-slate-500 mb-2"># HIRING_SIGNALS</div>
                                    <TermRow label="EST_COMP" value={<span className="text-emerald-400 font-bold">{result.hiring_and_compensation?.estimated_compensation_range}</span>} delay="2500ms" />
                                    <TermRow label="TRENDS" value={<TypewriterText text={result.hiring_and_compensation?.fresher_hiring_trends} delayOffset={2600} highlightKeywords={keywords} />} delay="2600ms" />
                                </div>
                            </div>

                            <div className="space-y-3 term-animate" style={{ animationDelay: '3500ms' }}>
                                <div className="text-cyan-600"># TECHNICAL_REQUIREMENTS</div>
                                <TermRow label="SYS_DESIGN" value={<TypewriterText text={result.technical_interview_focus?.system_design_expectations} delayOffset={3600} highlightKeywords={keywords} />} delay="3600ms" />
                                <TermArray label="DEEP_DIVES" items={result.technical_interview_focus?.core_tech_stack_deep_dives} colorClass="border-cyan-800 text-cyan-400 bg-cyan-950" delay="4000ms" />
                                <TermArray label="DS_ALGO" items={result.technical_interview_focus?.data_structures_and_algorithms} colorClass="border-orange-800 text-orange-400 bg-orange-950" delay="4200ms" />
                            </div>

                            <div className="pt-4 term-animate text-slate-500" style={{ animationDelay: '4500ms' }}>
